 ...
 */
@Config
@TeleOp(name = "FieldVersatilityOpMode", group = "testing")
@Disabled
public class FieldVersatilityOpMode extends LinearOpMode
{
    ...
    public void runOpMode() throws InterruptedException
    {
        ...
        while (opModeIsActive())
        {
            ...
            // drawing an orbiting triangle pointing mostly up the X axis to indicate field theta =
            ...
            telemetry.addData("x",
                    AMPLITUDE * Math
                            .sin(2 * Math.PI * FREQUENCY * (System.currentTimeMillis() / 1000d)
            ...
            packet.fieldOverlay()
                    ...
                    // images, text and grids by default are drawn using the page transform where
                    // 0,0 is top left and 144,144 is bottom right
                    // but extra parameters are available to allow rotation and drawing in the
                    // current transform
                    // all other drawing primitives are rendered in the current (or default)
                    // transform, as built using setTranslation, setRotation and setScale
                    .setAlpha(.25).drawImage(ALTIMGSRC, ALTIMGX, ALTIMGY, ALTIMGW, ALTIMGH)

                    // optionally add custom gridlines, minimum of 2 to render field edges, anything
                    // less suppresses gridlines in that direction, default is 7
                    .setAlpha(1.0)
                    ...
                    // you can draw multiple images and can rotate them around a specified
                    // anchor/pivot point and draw them in the current transform instead of the page
                    // frame
                    .drawImage("/dash/ftc.jpg", 24, 24, 48, 48, 0, 0, 0, false)

                    // demonstrate an alternate transform to move the origin and orientation
                    // default origin for dashboard is in the center of the field with X axis
                    // pointing up
                    ...
                    // .setRotation(-Math.PI/4) //uncomment to see a rotation of 45 degrees, there
                    // have been FTC games with a diagonal field symmetry

                    .setScale(SCALEX, SCALEY) // be sure the vales evaluate to a doubles and not
                    // ints
                    // .setScale(144.0/105,144.0/105) //example of FIFA soccer field in meters

                    // draw the axes of the new origin
                    // the text labels will be drawn in the current origin transform, not in the
                    // page frame
                    .setStrokeWidth(1).setStroke("green").strokeLine(0, 0, 0, 24) // y axis
                    .setFill("green")
                    .strokeText("Y axis", 0, (RED_ALLIANCE ? 24 : 0), "8px serif",
                            -Math.PI / 2 * (RED_ALLIANCE ? -1 : 1), false)
                    .setStroke("red").strokeLine(0, 0, 24, 0) // x axis
                    .setFill("red").fillText("X axis", 0, 0, "8px Arial", 0, false)

                    .setStroke("goldenrod").strokeCircle(0, 0, ORBITAL_RADIUS).setFill("black")
                    .fillPolygon(bxPoints, byPoints).setFill("blue")
                    // label the arrow as pointing 15 degree counter clockwise
                    .fillText("15 deg CC", bx - 10, by, "8px Arial", Math.toRadians(90 - 15), false)
                    .setAlpha(.25)
                    // you can draw multiple images and can rotate them around a specified pivot
                    ...
                    .setAlpha(1.0).fillText(TXTTEXT, TXTX, TXTY, TXTFONT,
                            Math.toRadians(TXT_THETA_DEGREES), TXT_USE_PAGE_FRAME);
